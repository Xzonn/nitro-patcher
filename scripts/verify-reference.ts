...
import { createRom, makeZip, type ZipEntries } from '../test/fixtures';

const execFileAsync = promisify(execFile);
const reference = process.env.NITROPATCHER_REFERENCE;
if (!reference) {
  throw new Error(
    'NITROPATCHER_REFERENCE must name the executable original NitroPatcherCli launcher',
  );
}
...
const changedBanner = Buffer.from(
  baseRom.subarray(baseRom.readUInt32LE(0x68), baseRom.readUInt32LE(0x68) + 0x840),
);
...
try {
  for (const [index, testCase] of cases.entries()) {
    ...
    const status = stdout.match(/^(SUCCESS|MD5_MISMATCH)$/m)?.[1];
    const md5s: string[] = [...stdout.matchAll(/[0-9a-f]{32}/gi)].map((match) =>
      match[0]!.toLowerCase(),
    );
    assert.ok(
      status,
      `${testCase.name}: could not parse original CLI status from ${JSON.stringify(stdout)}`,
    );
