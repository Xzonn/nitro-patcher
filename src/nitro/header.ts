...
export class Header {
  ...
  constructor(input: Uint8Array) {
    ...
    this.trimmedRom = this.total_rom_size ? this.total_rom_size >= data.length : data.length !== this.size;
    this.doublePadding =
      (this.ARM9size % 0x400 < 0x200 && this.ARM7romOffset % 0x400 === 0 && this.ARM9overlayOffset % 0x400 === 0) ||
      (this.ARM7size % 0x400 < 0x200 && this.FNToffset % 0x400 === 0 && this.ARM7overlayOffset % 0x400 === 0) ||
    ...
    this.nitrocode =
      nitroOffset >= size && nitroOffset + 4 <= data.length && data.readUInt32LE(nitroOffset) === 0xdec00621;
    ...
    this.secureCRC =
      secure.length > 0 &&
      crc16(this.decrypted ? encryptSecureArea(this.gameCode, secure) : secure) === this.secureCRC16;
    this.dlp_signature = Buffer.alloc(0);
    if (this.ROMsize >= size && this.ROMsize + 0x88 <= data.length && data.readUInt32LE(this.ROMsize) === 0x00016361) {
