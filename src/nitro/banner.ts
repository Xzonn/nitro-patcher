...
export class Banner {
  ...
  constructor(input: Uint8Array, offset = 0, hardSize = 0) {
    ...
    if (this.version >>> 8 === 1)
      size = hardSize > 0 && hardSize !== 0xffffffff ? Math.min(hardSize, 0x23c0) : 0x23c0;
  ...
  toBuffer() {
    ...
    if (this.tileData.length !== 512 || this.palette.length !== 32)
      throw new RangeError('Invalid banner icon size');
    ...
    for (let i = 0; i < this.titleCount; i++) {
      ...
      if (text.length > 256)
        throw new RangeError(`Banner ${titles[i]} exceeds 128 UTF-16 code units`);
