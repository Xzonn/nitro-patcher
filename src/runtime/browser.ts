...
export const hmacSha1 = (key: Uint8Array, data: Uint8Array): Buffer => Buffer.from(hmac(sha1, key, data));
...
export const inflateRawSync = (data: Uint8Array, options: { maxOutputLength: number }): Buffer => {
  ...
  const stream = new Inflate((chunk) => {
    if (chunk.length > output.length - length) throw new RangeError("Inflated data exceeds declared size");
