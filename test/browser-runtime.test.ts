...
import { createAesBlockCipher, hashDigest, hmacSha1, inflateRawSync } from "../src/runtime/browser";
...
test("browser inflation handles empty data and enforces output bounds", () => {
  for (const size of [0, 1, 32768, 131073]) {
    ...
    if (size > 1) assert.throws(() => inflateRawSync(compressed, { maxOutputLength: size - 1 }), /exceeds/);
  }
