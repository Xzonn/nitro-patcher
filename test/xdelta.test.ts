...
test("independent literal VCDIFF vector decodes Hello", () => {
  assert.equal(
    decodeXdelta(Buffer.alloc(0), Buffer.from("d6c3c40000000b050005010048656c6c6f06", "hex")).toString(),
    "Hello",
  );
});
...
test("HERE, all NEAR slots, and all SAME banks resolve cached source addresses", () => {
  ...
  const inst = [19, 1, 19, 1, 19, 1, 19, 1, 51, 1, 67, 1, 83, 1, 99, 1, 115, 1, 131, 1, 147, 1, 35, 1];
  const addr = [...vint(1), ...vint(257), ...vint(513), ...vint(7), 0, 0, 0, 0, 1, 1, 1, ...vint(12)];
  assert.deepEqual(
...
test("paired ADD/COPY and COPY/ADD opcodes decode in order", () => {
  const p = patch(window({ flags: 1, sourceSize: 4, size: 10, data: [33, 63], inst: [163, 247], addr: [0, 0] }));
...
test("rejects truncated streams and malformed lengths without partial output", () => {
  ...
  for (let i = 0; i < p.length; i++) {
    if (i !== header.length) assert.throws(() => decodeXdelta(Buffer.alloc(0), p.subarray(0, i)), /VCDIFF/);
  }
  assert.throws(
...
test("explicitly rejects unsupported compression, custom tables, versions and indicator bits", () => {
  for (const [h, message] of [
    ...
    assert.throws(() => decodeXdelta(Buffer.alloc(0), Buffer.from([0xd6, 0xc3, 0xc4, 0, h])), message);
  }
  assert.throws(() => decodeXdelta(Buffer.alloc(0), Buffer.from([0xd6, 0xc3, 0xc4, 1, 0])), /version/i);
  assert.throws(() => decodeXdelta(Buffer.alloc(0), patch(window({ flags: 3, size: 0 }))), /indicator|source/i);
});
