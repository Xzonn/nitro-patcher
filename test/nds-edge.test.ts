...
test('full DSi ROM serialization preserves payloads and all digest levels repeatedly', () => {
  ...
  const block = serialized.subarray(
    h.block_hashtable_start,
    h.block_hashtable_start + h.block_hashtable_size,
  );
  assert.deepEqual(h.hmac_digest_master, hmac(block));
  assert.deepEqual(
    block,
    hmac(serialized.subarray(h.sector_hashtable_start, h.sector_hashtable_start + 0x280)),
  );
  const ntrFirst = serialized.subarray(h.digest_ntr_start, h.digest_ntr_start + 0x400);
  assert.deepEqual(
    serialized.subarray(h.sector_hashtable_start, h.sector_hashtable_start + 20),
    hmac(ntrFirst),
  );
